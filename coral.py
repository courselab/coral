...
>>>>>>> 8e58ce496b9164278e6676f0b8b797b9e727c10a
...
# Draw stylized head 
def draw_head(self):
    # Define the size of the grid and head dimensions
    GRID_SIZE = size[configs[1]]  # Grid size from the configuration
    head_radius = GRID_SIZE // 2  # Radius of the head (half the grid size)
    head_center = (self.head.x + head_radius, self.head.y + head_radius)  # Center of the head (adjusted for radius)
    
    # Choose the color of the head based on whether the snake is alive or not
    head_color = HEAD_COLOR if self.alive else DEAD_HEAD_COLOR
    
    # Draw the rounded head of the snake using a circle
    pygame.draw.circle(arena, head_color, head_center, head_radius)
    
    # Define the eye offset based on the head radius
    eye_offset = head_radius // 2
    
    # Based on the direction the snake is moving, adjust the position of the body and other features
    if self.xmov == 1:  # Moving right
        body_rect = pygame.Rect(self.head.x, self.head.y, GRID_SIZE // 2, GRID_SIZE)  # Rectangular body part
        right_eye = (eye_offset, -eye_offset)
        left_eye = (eye_offset, eye_offset)
        tongue_pos = (head_center[0] + head_radius, head_center[1])  # Position for the tongue
        tongue_direction = (10, 2)  # Direction of the tongue (horizontal)
    elif self.xmov == -1:  # Moving left
        body_rect = pygame.Rect(self.head.x + head_radius, self.head.y, GRID_SIZE // 2, GRID_SIZE)
        right_eye = (-eye_offset, -eye_offset)
        left_eye = (-eye_offset, eye_offset)
        tongue_pos = (head_center[0] - 3 / 2 * head_radius, head_center[1])  # Adjust tongue position
        ...
        tongue_direction = (2, 10)  # Direction for vertical tongue
    else:  # Moving up
    ...
    # Draw the rectangular body part behind the head based on direction
    pygame.draw.rect(arena, head_color, body_rect)

    # Define the radius for eyes
    eye_radius = 7
    ...
    # Draw eyes based on whether the snake is alive
    if self.alive:
        pupil_radius = 4
        ...
        eye_line_length = 3
        # Draw the "X" shape in the eyes for dead snake
        pygame.draw.circle(arena, "#FFFFFF", left_eye_pos, eye_radius)
        pygame.draw.circle(arena, "#FFFFFF", right_eye_pos, eye_radius)
        # Draw crossed lines in the eyes to indicate death
    ...
    # Randomly display the snake's tongue (chance controlled by random)
    if self.alive and random.randint(0, 10) > 8:  # Adjust the chance of appearance here
        pygame.draw.rect(arena, "#FF0000", pygame.Rect(tongue_pos, tongue_direction))

# Draw stylized tail
def draw_tail(self, tail, direction):
    # Define tail dimensions based on the grid size
    ...
    # Determine the shape and position of the tail based on the last segment's movement
    ...
    # Choose color for the tail based on snake's alive status
    tail_color = HEAD_COLOR if self.alive else DEAD_HEAD_COLOR

    # Draw the main rounded part of the tail
    pygame.draw.circle(arena, tail_color, tail_center, tail_radius)

    # Draw the rectangular part connecting to the next segment of the tail
