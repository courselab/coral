...
class BaseFruit:
    def __init__(self, color, snake, obstacles) -> None:
        self.color = color
        ...
        while snake.is_in_position(self.x, self.y) or any(
            obstacle.is_in_position(self.x, self.y) for obstacle in obstacles
        ):
            # Prevent fruits from spawning on top of the snake
            self.x = int(random.randint(0, WIDTH) / size[configs[1]]) * size[configs[1]]
            self.y = (
                int(random.randint(0, HEIGHT) / size[configs[1]]) * size[configs[1]]
            )
    ...
    def update(self):
        pygame.draw.circle(
            gm.arena, self.color, (self.rect.centerx, self.rect.centery), self.radius
        )
